...
def get_filters():
    ...
    while True:
        ...
        if not city_found:
            city = input("We have 3 cities available to explore : Chicago," +
                         "Washington, New York City. Please choose one : ")
            city = city.lower()
            if city not in CITY_DATA:
                print("Invalid city or data not available, please choose " +
                      " one of the 3 : Chicago, Washington, New York City")
                continue
            else:
                city_found = True
        ...
        if not month_found:
            month = input("Enter month you want to explore. Choose one of : "
                          ...
                          "NOV, DEC, ALL. ALL denotes data for all months : ")
            month = month.upper()
        ...
        day = input("Enter day you want to explore. Choose one of : " +
                    "MON, TUE, WED, THU, FRI, SAT, SUN, ALL. ALL " +
                    "denotes data for all days :")
        day = day.upper()
...
def load_data(city, month, day):
    ...
    # extract start month from the Start time column to create Start
    # Month column
    ...
    df['Start Day'] = pd.to_datetime(df['Start Time'], format='%Y-%m-%d %H:%M:%S').dt.dayofweek  # NOQA

    # extract start hour from the Start Time column to create an Start
    # Hour column
...
def time_stats(df, month, day):
    ...
    if month == MONTHS_TO_INDEX.get('ALL'):
        ...
        if popular_month.empty:
            print("No popular month found for the filter specified!! " +
                  "Please adjust your filter!!")
        else:
            popular_month = popular_month.mode()[0]
            print('Most popular month for renting is : ' +
                  '{}'.format(INDEX_TO_MONTH.get(popular_month)))
    else:
        print('As you have chosen month : {} as filter, most popular month for renting won\'t be calculated'.format(month))  # NOQA

    # display the most common day of week
    if day == DAY_T0_INDEX.get('ALL'):
        popular_day = df['Start Day'].dropna()  # .mode()[0]
        if popular_day.empty:
            print('No popular day found for the filters specified!! ' +
                  'Please adjust your filter!!!')
        else:
            popular_day = popular_day.mode()[0]
            print('Most popular day for renting is ' +
                  ' : {}'.format(INDEX_TO_DAY.get(popular_day)))
    else:
            print('As you have chosen "{}day" as filter, most popular day for renting won\'t be calculated'.format(day.title()))   # NOQA

    # display the most common start hour
    popular_start_hour = df['Start Hour'].dropna()
    if popular_start_hour.empty:
        print('No popular start hour found for the filter specified!!' +
              ' Please adjust your filter !!!')
    else:
        popular_start_hour = popular_start_hour.mode()[0]
        print('Most popular renting start hour is :' +
              ' {}:00 hrs'.format(popular_start_hour))
...
def station_stats(df):
    ...
    if most_popular_start_station.empty:
        print('No \'Start Station\' data found for the filter specified!! ' +
              'Kindly adjust your filter')
    else:
        most_popular_start_station = most_popular_start_station.mode()[0]
        print('Most popular start station for the filter specified is : ' +
              '{}'.format(most_popular_start_station))

    # display most commonly used end station
    most_popular_end_station = df['End Station']
    if most_popular_end_station.empty:
        print('No \'End Station\' data found for the filter specified!! ' +
              ' Kindly adjust your filter')
    else:
        most_popular_end_station = most_popular_end_station.mode()[0]
        print('Most popular end station for the filter specified is :' +
              ' {}'.format(most_popular_end_station))

    # display most frequent combination of start station and end station trip
        most_frequent_start_and_end_station = df[['Start Station', 'End Station']].dropna()  # NOQA
    if most_frequent_start_and_end_station.empty:
        print('No data found for the filter specified!! Kindly adjust your filter')   # NOQA
    else:
        most_frequent_start_and_end_station = most_frequent_start_and_end_station.groupby(['Start Station', 'End Station']).size().sort_values(ascending=False)  # NOQA
        trip_count = most_frequent_start_and_end_station.iloc[0]
        stations = most_frequent_start_and_end_station[most_frequent_start_and_end_station == trip_count].index[0]  # NOQA

        start_station, end_station = stations
        print('Most frequent start station : {} and end station {} which was part of trips {} times'.format(start_station, end_station, trip_count))  # NOQA
...
def user_stats(df):
    ...
    if user_type.empty:
        print('No data available for specified filter, ' +
              'please adjust your filter!!')
    else:
        user_type = user_type.value_counts()
        print('User type details for the filter specified : ' +
              '{}'.format(user_type))
    ...
    if 'Birth Year' in df:
        ...
        if birth_years.empty:
            ...
            if user_birth_year.empty:
                print('No data available for your filter, ' +
                      'please adjust your filter!!!')
            else:
                oldest_user = user_birth_year.min()
                print('Earliest year of birth for the selected filter : {}'
                      .format(int(oldest_user)))

                youngest_user = user_birth_year.max()
                print('Most recent year of birth for the selected filter : {}'
                      .format(int(youngest_user)))

                most_common_year_of_birth = user_birth_year.mode()[0]
                print('Most common year of birth for the selected filter : {}'
...
def main():
    while True:
        ...
        print("Input to be processed -> City : {}, Month : {}, " +
              "Day : {}".format(city, month, day))

        df = load_data(city, month, day)

        if df.empty:
            print('No data found for specified filter, please adjust' +
                  ' your filters!!!')
            continue
